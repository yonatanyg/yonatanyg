<h1 align="center">Hi, I'm Yonatan 👋</h1>
<h3 align="center">Software Developer • Computer Science Graduate • Game Dev Enthusiast</h3>

<p align="center">
  <a href="https://github.com/yonatanyg">
    <img src="https://img.shields.io/github/followers/yonatanyg?label=Followers&style=social" />
  </a>
</p>

---

## 🧠 About Me

🎓 Computer Science graduate from the Hebrew University of Jerusalem  
💻 Software developer who enjoys building systems from the ground up  
🎮 With a special place at heart for game dev  

I like projects that mix **logic, creativity, and clean architecture** — whether it’s a backend service, a full-stack app, or an interactive game mechanic.

---

## 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,js,ts,react,fastapi,docker,postgres,unity,cs,java&perline=5" />
</p>

---

## 📌 Featured Projects

- 🧠 **Full-Stack Trivia App**  
  React • FastAPI • PostgreSQL • Docker

- 📈 **Bitcoin Tracker**  
  Real-time data processing & clean API design

- 🎮 **Game Development Projects**  
  2D mechanics, physics interactions, and level design

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=yonatanyg&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=yonatanyg&layout=compact&theme=tokyonight" />
</p>

---

## 📫 Connect With Me

<p align="center">
  <a href="https://github.com/yonatanyg">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github" />
  </a>
  <!-- Add LinkedIn when ready -->
</p>

---

<p align="center">
  <i>Always curious. Always building.</i>
</p>
